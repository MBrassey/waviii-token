...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    const transactions = await waviii.getPastEvents('Transfer', { fromBlock: 0, toBlock: 'latest', filter: { from: this.state.account } })
    this.setState({ transactions: transactions })
    console.log(transactions)
  }

  transfer(recipient, amount) {
    this.state.waviii.methods.transfer(recipient, amount).send({ from: this.state.account })
  }
