...
contract('waviii', function(accounts) {
  ...
  it('allocates the initial supply upon deployment', function() {
    return waviii.deployed().then(function(instance) {
      ...
      assert.equal(totalSupply.toString(), 1000000, 'sets the total supply to 1,000,000');
      return tokenInstance.balanceOf(accounts[0]);
    }).then(function(adminBalance) {
      assert.equal(adminBalance.toString(), 1000000, 'it allocates the initial supply to the admin account');
    });
  });
