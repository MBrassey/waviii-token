...
contract('waviii', function(accounts) {
  ...
  it('allocates the initial token supply to Luc1d upon deployment', function() {
    return waviii.deployed().then(function(instance) {
      tokenInstance = instance;
      return tokenInstance.totalSupply();
    }).then(function(totalSupply) {
      assert.equal(totalSupply.toNumber(), 1000000000000000000000000, 'sets the total supply to the initial supply');
      return tokenInstance.balanceOf(accounts[0]);
    }).then(function(adminBalance) {
      assert.equal(adminBalance.toNumber(), 1000000000000000000000000, 'it allocates the initial supply to the admin account');
    });
  });
